...
class Brain {
  ...
  feed(inputValues) {
    ...
    // Debug: Print entry neurons before update
    console.log(
      `Entry Neurons before update: ${JSON.stringify(this.entryNeurons)}`,
    );

    // Set input values to entry neurons
    this.entryNeurons.forEach((neuron, index) => {
      if (neuron) {
        neuron.potential = (neuron.potential || 0) + inputValues[index];
      }
    });

    // Debug: Print entry neurons after update
    console.log(
      `Entry Neurons after update: ${JSON.stringify(this.entryNeurons)}`,
    );
  ...
  loadModel(filePath) {
    ...
    // Restore neurons with their properties
    this.neurons = model.neurons.map((neuron) => ({
      id: neuron.id,
      position: neuron.position,
      potential: neuron.potential, // Ensure potential is restored
    ...
    // Restore entry neurons and exit neuron by id
